...
###These are ppredictions of maternal deaths utilizing the models of with and without offset by each individual fixed effect (Ethnicity, Age Group, Dobbs Era) as you can see there are vast differences between the plots with offset and without offfset
...
plot(pred_dobbs) + ggtitle("Predicted Maternal Deaths by Dobbs Era (wthout offset")

pred_dobbs <- ggpredict(allno_glmmodel_nb, terms = "Dobbs_Era")
plot(pred_dobbs) + ggtitle("Predicted Maternal Deaths by Dobbs Era (with offset")
...
### This utilizes the offset model to predict maternal deaths over years by ethnicity
```{r}
library(ggplot2)
