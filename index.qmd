...
```
 ...
 - But even with low R² the statistical significance of the fixed effects is still important because it highlights the significant relationship between the explanatory variables and maternal mortality. We are not so focused on prediction where a higher R² would be more necessary for an adequate model
...
The residual variance is 0.01, which is  variance in maternal deaths not explained by fixed effects or random effects, a evidences a model that is extremely accurate.
Additionally, ICC- intra-class correlation is, 0.71, which is the proportion of the total variance explained by year and is suspiciously high.

This non- offset model is also too good to be true because the marginal R² is extremely high at, 97.1%, the proportion of variance explained by fixed effects only, and the conditional R² is 99.2%, the proportion of variance contributed by fixed and random effects. Inflated R² is further evidence of unaccounted exposure in a model without offset is a model that cannot be trusted to convey accurate model information. 
...
```
  ...
  This tests if model is overdispersed since the ratio given is around 1 and provides evidence that there is no  overdispersion and neither is the p value which is much greater that (p <0.05) which also indicates there is no significant overdispersion with this model.
