...
```{r}
sjPlot::plot_model(allno_glmmodel_nb, type = "eff", terms = c("Ethnicity", "Age_Group", "Dobbs_Era"))
```



```{r}
ggplot(deaths_df3, aes(x = Year, y = Maternal_Mortality_Rate, color = Ethnicity)) +
  geom_point() +
...
ggplot(deaths_df3, aes(x = Dobbs_Era, y = Maternal_Deaths, color = Ethnicity)) +
  ...
  geom_line(aes(group = Ethnicity)) +
  facet_wrap(~ Ethnicity) +
  labs(title = "Maternal Deaths Over Time by Ethnicity Pre v. Post Dobs Era")
```
...
```{r}
library(ggeffects)
ethnicity_effect <- ggpredict(all_glmmodel_nb, terms = "Ethnicity")

plot(ethnicity_effect) +
  ggtitle("Predicted Maternal Deaths by Ethnicity (with offset)")

ethnicity_effect2<-ggpredict(allno_glmmodel_nb, terms = "Ethnicity")
plot(ethnicity_effect2) +
  ggtitle("Predicted Maternal Deaths by Ethnicity (without offset)")
```
...
plot(ethnicityage_effect) +
  ggtitle("Predicted Maternal Deaths by Ethnicity ange age group (with offset)")
...
##AS you can see plottting without offset leads to faulty plots
library(ggeffects)
ethnicityagedobbs_effect <- ggpredict(all_glmmodel_nb, terms = c("Ethnicity","Age_Group", "Dobbs_Era"))

plot(ethnicityagedobbs_effect) +
  ggtitle("Predicted Maternal Deaths by Ethnicity ange age group (with offset)")

ethnicityagedobbs_effect2<-ggpredict(allno_glmmodel_nb, terms = c("Ethnicity", "Age_Group", "Dobbs_Era"))
plot(ethnicityagedobbs_effect2) +
  ggtitle("Predicted Maternal Deaths by Ethnicity (without offset)")
```
...
ggplot(deaths_df3, aes(x = as.numeric(as.character(Year)), y = Maternal_Mortality_Rate, color = Ethnicity)) +
  ...
  facet_wrap(~Dobbs_Era) +
  labs(
    title = "Maternal Mortality Rate Over Time by Ethnicity and Dobbs Era",
    x = "Year",
    y = "Maternal Mortaity Rate (per 100,000 live births)"
  ) +
  theme_minimal()
