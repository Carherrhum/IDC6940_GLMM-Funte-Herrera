...
change_df <- test_df %>%
    mutate(Subgroup = str_trim(Subgroup),
  ethnicity= coalesce(case_when
  (Subgroup %in% c(
    "Asian, Non-Hispanic",
    "Black, Non-Hispanic",
    "White, Non-Hispanic",
    ...
    age_group = coalesce(case_when(
      Subgroup %in% c("Under 25 years", "25-39 years", "40 years and over") ~ Subgroup,
      TRUE ~ NA_character_
    )),
     total = case_when(
...
```{r}
poissonmodel_glmm_age <- glmer(Maternal_Deaths ~ age_group + (1|Year),
                              offset=log(Live_Births),
                              family = poisson(link = "log"), 
                              data = change_df)
poissonmodel_glmm_age
```
...
```
```{r}
poissonmodel_glmm_age_nooffset <- glmer(Maternal_Deaths ~ age_group + (1|Year),
                              family = poisson(link = "log"), 
                              data = change_df)
poissonmodel_glmm_age_nooffset
```
...
check_overdispersion(poissonmodel_glmm_age)
check_overdispersion(poissonmodel_glmm_age_nooffset)
```
...
```{r}
age_glmmodel_nb <- glmmTMB(
  Maternal_Deaths ~  age_group+  (1 | Year),
  offset=log(Live_Births),
  family = nbinom2,
  data = change_df
)
summary(age_glmmodel_nb)
```
...
```{r}
#without offset
age_glmmodel_nb2 <- glmmTMB(
  Maternal_Deaths ~  age_group + (1 | Year),
  family = nbinom2,
  data = change_df
)
summary(age_glmmodel_nb2)
```
...
performance::check_model(age_glmmodel_nb)
performance::check_model(ethnicityonly_glmmodel_nb2)
performance::check_model(age_glmmodel_nb2)
```
```{r}
...
plot(sim2)
sim3 <- simulateResiduals(age_glmmodel_nb)
plot(sim3)
sim4 <- simulateResiduals(age_glmmodel_nb2)
plot(sim4)
```

```{r}
tm <-tab_model(ethnicityonly_glmmodel_nb)
tm
tm2<-tab_model(ethnicityonly_glmmodel_nb2)
tm2
tm3<-tab_model(age_glmmodel_nb)
tm3
tm4<-tab_model(age_glmmodel_nb2)
tm4
```
