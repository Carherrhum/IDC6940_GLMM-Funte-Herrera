...
change_df <- test_df %>% filter(Subgroup != "Total")
change_df<- change_df%>% mutate(age_group= case_when(
    str_detect(Subgroup, "Under 25") ~ "Under 25",
    str_detect(Subgroup, "25-39") ~ "25-39",
    str_detect(Subgroup, "40 years and over") ~ "40+",
    TRUE ~ NA_character_
  ))
...
change_df <- change_df %>%
    mutate(
  ethnicity= case_when
  (str_detect(Subgroup, "Asian, Non-Hispanic") ~ "Asian, Non-Hispanic",
    str_detect(Subgroup, "Black, Non-Hispanic") ~ "Black, Non-Hispanic",
    str_detect(Subgroup, "White, Non-Hispanic") ~ "White, Non-Hispanic",
    str_detect(Subgroup,"Hispanic") ~ "Hispanic",
    str_detect(Subgroup,"American Indian or Alaska Native, Non-Hispanic") ~"American Indian or Alaska Native, Non-Hispanic",
    str_detect(Subgroup, "Native Hawaiian or Other Pacific Islander, Non-Hispanic") ~ "Native Hawaiian or Other Pacific Islander, Non-Hispanic",
      TRUE ~ NA_character_
    ))
change_df$ethnicity <- as.factor(change_df$ethnicity)
change_df$ethnicity <- droplevels(change_df$ethnicity)
change_df$age_group <- as.factor(change_df$age_group)
change_df$age_group <- droplevels(change_df$age_group)
change_df <- change_df %>%
  mutate(Year = as.factor(Year))%>%
  mutate(Month= as.factor(Month))
df_filtered <- change_df %>% filter(Subgroup != "Total")
```

```{r}
poissonmodel_glmm_ethnicity <- glmer(Maternal_Deaths ~ ethnicity  + (1 | Year), 
...
```{r}
poissonmodel_glmm_both <- glmer(Maternal_Deaths ~ Subgroup + (1|Year),
                              offset=log(Live_Births),
                              family = poisson(link = "log"), 
                              data = test_df)
...
```
```{r}
poissonmodel_glmm_both_nooffset <- glmer(Maternal_Deaths ~ Subgroup + (1|Year),
                              family = poisson(link = "log"), 
                              data = test_df)
