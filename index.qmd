...
###checking performance, R2 and singlulairty of effect variable and collinearity of model without offset

```{r}
#checking performance, R2 and singlulairty of effect variable and colliarnity of model without offset
...
```
###The assumption graphs are the same for both models with and without offset. I included only those with offset cof conciseness. These are assumptions that need to be met for model to be acceptable.

##According to this graph The response variable and the predictors have a linear relationship within the levels of random effect
...
### The Independence of residual and Random Effects assummption is met because the dots of the residuals of the random effect (year) falls on the red refence line of  quantiles of random interceppt estimates vesus quantiles of normal distribution. Additionally a blue Loess lines is approximately close to this red reference lines which would also indicate independene of residuals and random effects.
```{r}
...
###This tests if model is overdisersed since the ratio given is around ,0.051 it is not indicative of overdisppersion and neither is the p value which is much grater that (p <0.05) which also indicates there is no significant overdispersion with this model.
...
####The coefficients of not using an offset has affected the estimates in a poor manner
...
###These are ppredictions of maternal deaths utilizing the models of with and without offset by each individual fixed effect (Ethnicity, Age Group) as you can see there are vast differences between the plots with offset and without offfset. predictions by Dobbs was approximately similar so I included only offset
...
plot(pred_eth) + ggtitle("Predicted Maternal Deaths by Ethnicity (without offset")

pred_eth <- ggpredict(all_glmmodel_nb, terms = "Ethnicity")
plot(pred_eth) + ggtitle("Predicted Maternal Deaths by Ethnicity (with offset")

#  predictions by Age Group
pred_age <- ggpredict(allno_glmmodel_nb, terms = "Age_Group")
plot(pred_age) + ggtitle("Predicted Maternal Deaths by Age Group (without offset)")

pred_age <- ggpredict(all_glmmodel_nb, terms = "Age_Group")
plot(pred_age) + ggtitle("Predicted Maternal Deaths by Age Group (with offset")

```


###These are predictions of maternal deaths by using the models with and without offset and with predictions using the fixed effects of Ethnicity and age group
...
plot(ethnicityage_effect) +
  ggtitle("Predicted Maternal Deaths by Ethnicity and age group (with offset)")

ethnicityage_effect2<-ggpredict(allno_glmmodel_nb, terms = c("Ethnicity", "Age_Group"))
plot(ethnicityage_effect2) +
  ggtitle("Predicted Maternal Deaths by Ethnicity and age group (without offset)")


```

###These are predictions of maternal deaths by using the models with and without offset and with predictions using the fixed effects of Ethnicity and Dobbs Era

####SHOW Up pplease because strangest looking
...
plot(ethnicitydobbs_effect) +
 ggtitle("Predicted Maternal Deaths by Ethnicity and Dobbs Era (with offset)")

ethnicitydobbs_effect2<-ggpredict(allno_glmmodel_nb, terms = c("Ethnicity",  "Dobbs_Era"))
plot(ethnicitydobbs_effect2) +
ggtitle("Predicted Maternal Deaths by Ethnicity and Dobbs Era (without offset)")

```

### This utilizes the offset model to predict maternal deaths over years by ethnicity with offset and without offset
```{r}
library(ggplot2)
...
# Plot
plot(year_eth2) +
  labs(title = "Predicted Maternal Deaths Over Years by Ethnicity(without offset)",
       x = "Year", y = "Predicted Maternal Deaths") +
  theme_minimal()
```

###This is a plot of maternal deaths over time by Ethnicity facet wrapped by Ethnicty by only utilizing dataframe data
```{r}
ggplot(deaths_df3, aes(x = Year, y = Maternal_Deaths, color = Ethnicity)) +
  geom_point() +
...
ggplot(deaths_df3, aes(x = Year, y = Maternal_Deaths, color = Dobbs_Era)) +
  ...
  facet_wrap(~ Dobbs_Era) +
  labs(title = "Maternal Deaths Over Time by Dobbs Era")
```

###This is a plot of maternal deaths by Dobbs Era by  age group facet wrapped by age group by only utilizing dataframe data. Facet wrap is splitting plots into different factor levels of age group

```{r}
library(ggplot2)

ggplot(deaths_df3, aes(x = Dobbs_Era, y = Maternal_Deaths, color = Age_Group)) +
  geom_point() +
  geom_line(aes(group = Age_Group)) +
  facet_wrap(~ Age_Group) +
  labs(title = "Maternal Deaths (Pre V Post Dobbs Era by Age Group")
```
###This is a plot of maternal deaths by Dobbs Era by  Ethnicity facet wrapped by Ethnicity by only utilizing dataframe data facet wrap creates small plots for each level so maybe ethnicity is too much
```{r}
library(ggplot2)

ggplot(deaths_df3, aes(x = Dobbs_Era, y = Maternal_Deaths, color = Ethnicity)) +
  geom_point() +
  geom_line(aes(group = Ethnicity)) +
  facet_wrap(~ Ethnicity) +
  labs(title = "Maternal Deaths (Pre V Post Dobbs Era by Ethnicity")
```
...
##This is a trend of predicted maternal deaths by ethnicity over time by utilizing the model for prediction and  mean data for lines and points
...
ggplot(deaths_df3, aes(x = as.numeric(as.character(Year)), y = Maternal_Mortality_Rate, color = Ethnicity)) +
  ...
  theme_minimal()
```
#We used variable maternal mortality rate to see maternal mortality rate over time by ethnicity (facet wrapped by Dobbs Era) and by using dataframe data and averages to plot lines and points
...
ggplot(deaths_df3, aes(x = as.numeric(as.character(Year)), y = Maternal_Mortality_Rate, color = Ethnicity)) +
  ...
  facet_wrap(~Dobbs_Era)+
  labs(
    title = "Maternal Mortality Rate Over Time by Ethnicity(facet wrapped by Dobbs Era)",
    x = "Year",
    y = "Maternal Mortaity Rate (per 100,000 live births)"
  ) +
  theme_minimal()
...
ggplot(deaths_df3, aes(x = as.numeric(as.character(Year)), y = Maternal_Mortality_Rate, color = Age_Group)) +
  ...
  theme_minimal()
```
#We used variable maternal mortality rate to see maternal mortality rate over time by Dobbs Era and by using dataframe data and averages to plot lines and points
```{r}
library(ggplot2)
