...
```{r}
poissonmodel_glmm_all <- glmer(Maternal_Deaths ~ Ethnicity +Age_Group+ Dobbs_Era+(1|Year) +(1|Month),
...
allno_glmmodel_nb <- glmmTMB(
  Maternal_Deaths ~  Ethnicity + Age_Group+Year + (1 |Month/Year),
  family = nbinom2,
  data = deaths_df3
)
...
ethnicity_agegroupno_glmmodel_nb <- glmmTMB(
  Maternal_Deaths ~  Ethnicity+ Age_Group + (1 |Month/Year),
  family = nbinom2,
  data = deaths_df3
)
...
ethnicity_yearno_glmmodel_nb <- glmmTMB(
  Maternal_Deaths ~  Ethnicity + Year +  (1 |Month/Year),
  family = nbinom2,
  data = deaths_df3
)
...
yearno_glmmodel_nb <- glmmTMB(
  Maternal_Deaths ~  Year  +  (1 |Month/Year),
  family = nbinom2,
  data = deaths_df3
)
...
all2_glmmodel_nb <- glmer.nb(
  Maternal_Deaths ~  Ethnicity + Age_Group+Year + (1 |Month/Year),
  offset=log(Live_Births),
  family = nbinom2,
  data = deaths_df3
)
summary(all2_glmmodel_nb)
```
```{r}
eage_glmmodel_nb <- glmer.nb(
  Maternal_Deaths ~  Ethnicity + Age_Group+(1 |Month/Year),
  offset=log(Live_Births),
  family = nbinom2,
  data = deaths_df3
)
summary(eage_glmmodel_nb)
```
```{r}
allno2glmmmodel<-glmer.nb(
  Maternal_Deaths ~  Ethnicity + Age_Group+(1 |Month/Year),
  family = nbinom2,
  data = deaths_df3
)
summary(allno2glmmmodel)
```
...
performance::check_model(ethnicity_year_glmmodel_nb)#misspesified and zero inflation not happy
performance::check_model(ethnicity_glmmodel_nb)#misspesified dispersiion and zero inflation not happy
performance::check_model(year_glmmodel_nb)##misspesified dispersiion and zero inflation not happy
performance::check_model(allno_glmmodel_nb)#not terrible######interesting
performance::check_model(ethnicity_agegroupno_glmmodel_nb)#not best with offset
performance::check_model(ethnicity_yearno_glmmodel_nb)# not great
performance::check_model(ethnicityno_glmmodel_nb)# sae problem
performance::check_model(yearno_glmmodel_nb)#misspesified and zero inflation not happy
...
#LAPLACE -lme4 glmer.nb
sim <- simulateResiduals(fittedModel = all2_glmmodel_nb, plot=TRUE)#changed to use lmer4
plot(sim)

sim2 <- simulateResiduals(eage_glmmodel_nb, refit = FALSE)
plot(sim2)
sim3 <- simulateResiduals(allno2glmmmodel, refit= FALSE)#proble
plot(sim3)
...
#LA PLACE
sjPlot::tab_model(all2_glmmodel_nb)
```
```{r}
sjPlot::tab_model(eage_glmmodel_nb)
```
