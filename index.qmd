...
```{r}
sjPlot::plot_model(all_glmmodel_nb, type = "re") #no extreme points with random effects #with offset it is same as without offset
```
...
###The assumption graphs are the sae for both models with and without offset. I included only those with offset cof conciseness
