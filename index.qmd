...
The mean and variance are:
 ...
 - $\text{Var}(Y) = \mu + \frac{\mu^2}{k}$
  ...
  The dispersion parameter $k$ controls the degree of overdispersion. As $k \to \infty$, the variance approaches the mean, and the Negative Binomial converges to the Poisson distribution. However, when $k$ is small, the variance increases, indicating significant overdispersion, making the Negative Binomial distribution preferable to Poisson in such cases [@zuur2009mixed]. 
...
We ran models with and without the offset term log(Live_Births) to evaluate differences in model fit, particularly using Akaike Information Criterion (AIC) as the primary metric for comparison. A lower AIC indicates a better-fitting model.

The best-performing models included the fixed effects Ethnicity, Age_Group, and Dobbs_Era, with Year as a random effect. The second place would be models with fixed effects Ethnicity and Age group and random effect Year. All models dropped the Age Group Unknown factor from inclusion.

The model with the lowest AIC (4561.1) included all fixed effects but excluded the offset. The next-best model, with an AIC of 4568.0, included the same fixed effects with the offset. Despite the slightly higher AIC, the offset-included model is more appropriate and valid, as it accounts for differences in exposure across groups (i.e., varying numbers of live births).

Excluding the offset implies that all observations have equal exposure — suggesting, for instance, that every ethnic group has the same number of live births each year, which is not the case. In contrast, including the offset adjusts for the actual number of live births, yielding rate-based estimates that correctly reflect the underlying population structure. While the offset-free model appears better based on AIC alone, it leads to biased estimates due to ignoring exposure variability [@hilbe2011negative], [@nguyen2020guide], [@ha2023r].

The majority of our visualizations will be based on the model with all fixed effects and with offset. Visualizations based on the non-offset model are included later for comparison with the offset model.

Here is the equation for a our chosen model in regression equation format:
$$
\begin{align*}
...
Here is the equation for the non-offset model:
...
The graph shows the residual values of the model and the fitted values of the model and the scatter points are the fitted values of the model. The red reference line is where the residuals equal zero.There are no patterns with the scatter points which provides evidence that the response variable and the predictors have linear relationship within the levels of random effect (year). Additionally, The blue LOESS smoother line is close to the red reference line of zero, which provides further evidence of the linear relationship. The residuals are roughly centered around zero across the range of values. The variability of the residuals on the left of the graph suggests heteroscedasticity, which is expected. 
```{r}
...
plot(x, y, main = "Residuals vs Fitted with LOESS model") 
abline(h = 0, col = "red")
...
- The Independence of residual and Random Effects assumption is met 
- The graph below plots the points of the the random effect (year) residuals falls on the red reference line of random intercept quantiles estimates versus normal distribution quantiles  and the red reference line represent where random effect residual points would lie if random intercepts were normally distributed.  
- The Q-Q plot of the random intercepts for Year suggests the distribution of the random effects is approximately normal. There is slight deviations at the extremes due to the low number of years.


```{r}

re_year <- ranef(all_glmmodel_nb)$cond$Year[[1]] 

qqnorm(re_year, main = "Q-Q Plot of Random Intercepts in model(Year)")
qqline(re_year, col = "red")
...
- The graph plots a blue density curve of random effect distribution. The red dashed line of the distribution indicates the distribution of ideal normal distribution with same means and standard deviation. The blue density curve falls approximately within the red line so the random effects are approximately normal. The approximately normal blue density of the random effect(year) could be expected to have mild deviations since we are working with count data. Yet, since most of the blue density falls within the ideal normal distribution curve then there is normality of random effects
...
re_year <- ranef(all_glmmodel_nb)$cond$Year[["(Intercept)"]] #extract year random effect intercepts as numeric
...
This is a model summary output with beta coefficients exponentiation into incidence rate ratios.
<div class="scroll-box-vertical2">
```{r,results='asis'}
library(sjPlot)
...
##### Results Snapshot by Predictor (Offset Model)
...
The residual variance of the model is 7.99, representing the variability in maternal deaths not explained by either fixed or random effects. The marginal R², which reflects the proportion of variance explained by fixed effects alone, is 5.6%. This indicates that the fixed effects account for only a small portion of the variability in maternal deaths. The conditional R², which includes both fixed and random effects, is 5.9%. This slight increase suggests that the random effect of Year contributes minimally to improving the model’s explanatory power.
The low proportion of variance explained by the model may be due to the absence of additional random effects—such as State—or missing fixed effects that capture important factors like socioeconomic status, healthcare access, or prenatal care quality. Despite the low R² values, the statistical significance of the fixed effects remains meaningful, as it underscores the presence of significant associations between the explanatory variables and maternal mortality. Since our focus is not on prediction, but rather on understanding these relationships, a higher R² is not essential for the model to be informative.

Below is a brief glimpse at the non-offset GLMM model table estimates.
...
We chose not to interpret the incidence rate ratios (IRRs) from the non-offset model due to their extreme inflation. For example, the model suggests that White women experience a 98.8% higher maternal mortality rate compared to the Asian reference group. It also indicates that the fixed effect for the 40 years and older age group is not statistically significant, while the "Unknown" ethnicity is.

The intra-class correlation (ICC) is also suspiciously high at 0.71, implying that 71% of the total variance is attributed to the random effect Year.

Even more problematic is the model’s marginal R² of 95.7%, indicating that fixed effects alone explain nearly all of the variance in maternal mortality. The conditional R² rises to 98.8% when including random effects. These values are unrealistically high, and provide further evidence of unaccounted exposure in a model without offset.
...
- There is no singularity of the random effect (year)which demonstrates that the random effect year does contribute to the model. 
- The collinearity VIF numbers have low values which suggests that none of the predictor variables are highly correlated which each other
...
```
  ...
  This tests if the model is overdispersed since the ratio given is around 1 and provides evidence that there is no overdispersion and neither is the p value which is much greater that (p <0.05) which also indicates there is no significant overdispersion with this model.
...
```
 ...
 These are predictions of maternal deaths by individual fixed effect (Ethnicity, Age Group) for with and without offset models. As you can see, there are vast differences between the plots with and without offset. 
  Visually one can see without offset that the most maternal death counts are white women (who make up the majority of the population) followed by black women. However, with offset the exposure is clear and there can be brought evidence of the unfortunate inflated maternal mortality rates of American Indian or Alaska Native women and black women. 
...
```
  ...
  Without offset, the GLMM model implies that the population with the largest number of deaths (age group 25-39) contributes most to maternal mortality rate. However, the riskiest pregnancies 40 and over women, while still being the smallest population iving birth [@sheen2018maternal]. The plot of the model with offsets  shows clearly the risk to women over 40 giving birth. 
...
```
  ...
  These are predictions of maternal deaths by using the models with and without offset and with predictions using the fixed effects of Ethnicity and Age group. The model with offsets shows that the women at most risk for maternal death are black and American Indian or Native Alaskan over the age of 40, though the age range increased risk to all ethnicities[@macdorman2018trends]. 
  Without offsets, the model focuses on the age and ethnicities with the largest populations of women that give birth, such as white women aged 25-39, followed by women of black, Hispanic, and unknown race/ethnicities. American Indian or Alaska Native are shown to have very low mortality rate because it is a smaller population overall.
...
```
  ...
  This visualization utilizes the offset model to predict maternal deaths by ethnicity and shows the prediction of American Indian or Alaska Native women having the highest maternal mortality, followed by black women. In the non-offset model, again white women are shown to have highest maternal mortality followed by black women, with unknown and Hispanic women having around same rates and lowest is the low population of American Indian and Alaskan Native.
...
```
  ...
  We find that using an offset for maternal count data per live births provided a model that may not have been the highest performing but showed clear indications of at risk groups such as American Indian women, black women, and women over 40. The non-offset model may have deceptively had better performance numbers but implied that low-risk populations had higher mortality risk due to being a larger population. 
...
- Due to the Covid-19 pandemic's impact on the healthcare system, access to regular healthcare was restricted. This likely had an impact on maternal mortality and may partially account for increased rates from around 2022.

- More variables in the dataset would offer a better picture of the predictors of maternal mortality, specifically in regards to their relationship with ethnicity. Prenatal health, healthcare access, abortion access, and other prenatal behaviors would be useful[@chinn2020maternal].
 
### Conclusion

We initially hypothesized that our Dobbs Era variable would reveal changes in maternal mortality following the overturn of Roe v. Wade. However, our analysis is limited by the use of provisional data and a nationwide exposure, which may obscure more granular, state-level differences. Future models could improve on this by including State as a random effect or incorporating abortion access as a fixed effect. For example, abortion access could be categorized into three levels: extremely limited, somewhat limited, and no limitations. These additions could provide a more accurate picture of how maternal mortality has evolved in the post-Dobbs landscape.

Additionally, investigation into healthcare access for expecting mothers specific to the Covid-19 pandemic would give insight into the spike in maternal mortality rate in 2022. Given that mortality rate reaches its peak in February 2022 (months before Dobbs), it would be valuable to determine what effect is due to the pandemic in order to better evaluate the impact of the overturn of Roe v. Wade. 

  Regardless of these limitations, our results reaffirm a well-established and deeply concerning reality: minority women—particularly American Indian or Alaska Native women, and Black women—face significantly higher maternal mortality risks[@cusack2024maternal], [@tran2023disparities].
  
  In American Indian or Alaska Native communities, healthcare is often provided through the Indian Health Service (IHS)[@sequist2011trends], [@kruse2022indian]. While this care is free, it is not always well-resourced. Health professionals within IHS are often rotated between reservations, making it difficult for patients to build long-term relationships with providers. This lack of consistency may contribute to mistrust, underreporting of symptoms, and missed clinical cues, further elevating health risks.
  
  Similarly, Black women may struggle with issues in with the healthcare system[@tran2023disparities]. They may not be heard by their healthcare professionals and which leads to preventable complications and deaths during and after childbirth.
  
 The higher maternal mortality for women over the age of 40 is to be expected, as geriatric pregnancies (those over age 35) are high risk[@sheen2018maternal]. However, perhaps age is only one part of the picture. Women in this age range often face many life stressors, such as caring for both children and aging parents, while also experiencing age-related health changes that may further elevate pregnancy risks.
 
It’s important to note that our models cannot fully explain the why behind the disparities in mortality rate between these high risk populations[@chinn2020maternal]. We are not present in the delivery room, during prenatal appointments, or throughout the postpartum period—moments where critical complications can arise. Moreover, while some datasets track prenatal care and health behaviors, they often do not follow maternal outcomes beyond live birth, limiting our ability to connect prenatal risk to maternal death.
