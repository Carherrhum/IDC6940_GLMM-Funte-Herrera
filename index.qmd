...
The response variable and the predictors have a linear relationship within the levels of random effects. The blue LOESS line is close to the red reference line of zero, which indicates there not a non-linear relationship. The residuals are roughly centered around zero across the range of values. The variability of the residuals on the left of the graph suggests heteroscedasticity, which is expected. 
```{r}
