...
## Application to Social Science {.smaller}
...
- These findings are relevant because we could use them as evidence to help reduce maternal mortality amongst certain groups and potentially investigate if a post-Dobbs era could have resulted in a higher maternal mortality rate. 
...
- Analysis performed with R [@R-base]
...
sjPlot::tab_model(allno_glmmodel_nb,
          title = "Fixed Effects from GLMM without offset(Incidence Rate Ratios)")
```
</div>
...
## Conclusions and Further Study {.smaller}
...
- If this GLMM could be improved upon with new data then racial disparities in maternal mortality could be reduced by identifying significant explanatory variables
