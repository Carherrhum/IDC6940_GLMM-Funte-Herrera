...
var(deaths_df3$Maternal_Deaths)##variance is indeed greater than mean so Negative Binomial moel would be appropriate
mean(deaths_df3$Maternal_Deaths)
```


```{r}
install.packages("gglmmTMB")
```
...
check_singularity(all_glmmodel_nb)#random effect is not a singularity
check_collinearity(all_glmmodel_nb)

```

```{r}
performance::check_model(ethnicity_agegroup_glmmodel_nb, verbose=TRUE, residual_type = "normal")#interesting
performance::r2(ethnicity_agegroup_glmmodel_nb)
check_singularity(ethnicity_agegroup_glmmodel_nb)
check_collinearity(ethnicity_agegroup_glmmodel_nb)
```
...
```{r}
performance::check_model(ethnicity_agegroupno_glmmodel_nb, verbose=TRUE, residual_type = "normal")#interesting
performance::r2(ethnicity_agegroupno_glmmodel_nb)
check_singularity(ethnicity_agegroupno_glmmodel_nb)
check_collinearity(ethnicity_agegroupno_glmmodel_nb)
```
...
plot(x, y, main = "Residuals vs Fitted with LOESS") #LOESS is smoother line
...
```{r}
#Checking independence between residuals and random effects
#year residual mean centered around 0 with no clear pattern or group #Assumption is upheld
resid_vals <- residuals(allno_glmmodel_nb, type = "pearson")


library(ggplot2)


#residuals centered around zero- central tendency
ggplot(deaths_df3, aes(x = as.factor(Year), y = resid_vals)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.5) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Residuals by Year", y = "Residuals", x = "Year")
...
qqnorm(re_year, main = "Q-Q Plot of Random Effect (Year)")
qqline(re_year, col = "red")#points should roughly lie by this line if normal
hist(re_year, main = "Histogram of Random Effects", xlab = "Random effect (Year)")
...
ggplot(df_re, aes(x = re_year)) +
  ...
  ggtitle("Density of Random Effect (Year) with Normal Curve")
...
qqnorm(re_year, main = "Q-Q Plot of Random Intercepts (Year)")#plots quantiles of random intercept estimates vs quantiles of normal distriibution
